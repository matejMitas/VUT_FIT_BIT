...
**/
	...
	if (RootPtr == NULL) {
		return FALSE;
	} else if (RootPtr->Key == K) { // pokud strom je prvni polozka
		*Content = RootPtr->BSTNodeCont;
        return TRUE;			   	
	} else {
		// neni prvni, budeme se tak dlouho rekurzivne dlouho zanorovat, az to najdeme
		if (RootPtr->Key > K)
			return BSTSearch(RootPtr->LPtr, K, Content);
		// to same, ale pro pravou
		else 
...
**/
	...
	if (*RootPtr == NULL) {
		// nova polozka
		tBSTNodePtr new_item = (tBSTNodePtr) malloc(sizeof(struct tBSTNode));
		...
		*RootPtr = new_item;
		...
		if ((*RootPtr)->Key > K) // doleva
			BSTInsert(&(*RootPtr)->LPtr, K, Content);
		else if ((*RootPtr)->Key < K) // doprava
...
**/
	...
	if ((*RootPtr)->RPtr == NULL) { // jsme na konci
		// pres hodnoty
		PtrReplaced->Key = (*RootPtr)->Key;
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
		// uloz pointer, aby jsi ho potom mohl vymazat
		tBSTNodePtr tmp_item = *RootPtr;
		// napoj do listu
