...
VertexIndex gpu_computeGLVertexID(
  ...
  VertexIndex gl_VertexID = (indices) ? indices[vertexShaderInvocation] : vertexShaderInvocation;
  return gl_VertexID;
}
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  assert(head != NULL);
    ...
    // cteci hlava + offset hlavy + pocet prvku * index
  return (void *const) (
    (uintptr_t) (head->buffer) + 
    head->offset + 
    (head->stride * gl_VertexID)
  );
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex gl_VertexID = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
  void const *attr;
  // projedeme vsechny indexy
  for (int i = 0; i < MAX_ATTRIBUTES; i++) {
    // mame nakonfigurovano?
    if (!puller->heads[i].enabled)
      attr = NULL;
    else 
      attr = gpu_computeVertexAttributeDataPointer(&puller->heads[i], gl_VertexID);
    // priradime
...
///  - GPUVertexShaderInput()
  ...
  // vertex, ktery nacteme a spustime na nim shader
  VertexIndex gl_Vertex;
  // vysledek pullery
  GPUVertexPullerOutput puller_out;
  // data pro shader
  GPUVertexShaderInput to_shader;
  // prvni pozice
  VertexShaderInvocation vertex_index = baseVertexShaderInvocation;

  for (int i = 0; i < nofPrimitiveVertices; i++) {
    // vypocet index
    gl_Vertex = gpu_computeGLVertexID(puller->indices, vertex_index);
    // nacteme si data pres puller
    gpu_runVertexPuller(&puller_out, puller, vertex_index);
    // priprava volani pro shader
    to_shader.attributes = &puller_out;
    to_shader.gl_VertexID = gl_Vertex;
    // spustime vertex shader
    vertexShader(&primitive->vertices[i], &to_shader, gpu);
    // posun pro vypocet indexu
    vertex_index++;
    // pocet pouzitych verticies
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  // ulozeni polozek
  Vec2 const *v = vertices;

  // vzorec z wiki
  float bottom = (
    ((v[1].data[1] - v[2].data[1]) * (v[0].data[0]  - v[2].data[0])) +
    ((v[2].data[0] - v[1].data[0]) *(v[0].data[1]  - v[2].data[1]))
  );

  coords->data[0] = (((v[1].data[1] - v[2].data[1]) * (pixelCenter->data[0] - v[2].data[0]))  +
                     ((v[2].data[0] - v[1].data[0]) * (pixelCenter->data[1] - v[2].data[1]))) / bottom;
  coords->data[1] = (((v[2].data[1] - v[0].data[1]) * (pixelCenter->data[0] - v[2].data[0]))  +
                     ((v[0].data[0] - v[2].data[0]) * (pixelCenter->data[1] - v[2].data[1]))) / bottom;
